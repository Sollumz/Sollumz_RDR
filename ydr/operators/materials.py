...
class SOLLUMZ_OT_convert_active_material_to_selected(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        game = context.scene.sollum_game_type
        materials = shadermats
        if game == SollumzGame.RDR:
            materials = rdr_shadermats
        shader_name = materials[context.window_manager.sz_shader_material_index].value
        new_material = MaterialConverter(aobj, mat).convert(shader_name, game)
